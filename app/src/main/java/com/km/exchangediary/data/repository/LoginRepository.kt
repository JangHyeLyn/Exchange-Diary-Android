...
class LoginRepository(private val dataSource: ExchangeDiaryDataSource, private val localPreferences: LoginPreferences): CoroutineScope {
    ...
    suspend fun getUserTokenAtRemote(accessToken: String): String? =
        withContext(coroutineContext) {
            try {
                dataSource.getExchangeDiaryService().getJWT(LoginRequestBody(accessToken)).data?.token
